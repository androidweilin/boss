 ...
 */
public class MessageListActivity extends BaseAct implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case GET_MESSAGE:
                    if (state == 1) {
                        adapter = new SwipeAdapter(mContext, state, order, mListView.getRightViewWidth(), onRightclick);
                        mListView.setAdapter(adapter);
                    } else if (state == 2) {
                        adapter = new SwipeAdapter(mContext, state, leave, mListView.getRightViewWidth(), onRightclick);
                        mListView.setAdapter(adapter);
                    } else {
                        adapter = new SwipeAdapter(mContext, state, system, mListView.getRightViewWidth(), onRightclick);
                        mListView.setAdapter(adapter);
                    }
    ...
    private String id;
    private SwipeAdapter.IOnItemRightClickListener onRightclick = new SwipeAdapter.IOnItemRightClickListener() {
        @Override
...
//                id = system.get(position).get
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        if (state == 1) {
            ...
            intent.putExtra("userStatus", order.get(position).getOrderState());
