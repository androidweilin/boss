 ...
 * @author likaihang
 */
public class ShopDetailInfoActivity extends BaseAct implements OnClickListener, AdapterView.OnItemClickListener, ReboundScrollView.ScrollViewListener {
...
//    private TextView mAddress;
    ...
    private List<RapidlyBean.FastChatListBean> chatlist = new ArrayList<RapidlyBean.FastChatListBean>();
    ...
    private List<GuessLikeBean.LikeProBean> mDatas = new ArrayList<GuessLikeBean.LikeProBean>();//猜你喜欢集合
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter.setOnItemClickLitener(new GalleryOtherAdapter.OnItemClickLitener() {
            ...
            public void onItemClick(View view, int position) {
                Toast.makeText(ShopDetailInfoActivity.this, "" + position, Toast.LENGTH_SHORT).show();
            }
        });
        initErrorPage();
...
//        load();
    ...
    @Override
    protected void load() {
        requestShopDetailInfo(mCommodityID, CrashApplication.latitude + "", CrashApplication.longitude + "");
        getComment();//获取留言
        //猜你喜欢
        getGases(proName);
    }
    ...
    private void initViews() {
        ...
        distance = (TextView) findViewById(R.id.tv_goods_detail_distance);
...
//        mAddress.setOnClickListener(this);
        ...
        keyboardlayout.setOnKeyboardStateChangedListener(new KeyboardListenRelativeLayout.IOnKeyboardStateChangedListener() {
            @Override
            ...
            if (isLogin) {
                switch (id) {
                    ...
                    case R.id.id_goodsdetail_store_container://私聊
                        ...
                        startActivity(i);
                        break;
                    case R.id.id_goodsdetail_star_container://极速聊
                        if (isopen == 0) {
                    ...
                    case R.id.bt_leave_message_send://发表、回复留言
                        if (TextUtils.isEmpty(commentContent)) {
                            ...
                            if (isLeave) {
                                ...
                                replayMessage(commentContent, crComnId, objectUserId, objectId, String.valueOf(status));
                            } else {
                                replayMessage(commentContent, crComnId, objectUserId, objectId, String.valueOf(status));
                            }
                        }
...
//                //initUmengShareCompounets();
                UMImage urlImage = new UMImage(this, BitmapFactory.decodeResource(this.getResources(), R.drawable.ic_launcher));
                String uri = _url + comAndReply.getProId();
                umengShareUtils = new UmengShareUtils(this, desc, uri, comAndReply.getProName(), urlImage);
        ...
        if (chatlist.get(0).getStatus() == 1) {
            if (myTicket.equals("0")) {
                dialog.setClickListener(new onBtnClick() {
                    ...
                    @Override
                    public void buyBtn() {
                        Intent intent = new Intent(ShopDetailInfoActivity.this, BuyTicketActivity.class);
    ...
    private void getChatInfo() {
        ...
        try {
            ...
            obj.put("userid", comAndReply.getUserId());
        ...
        HttpUtils.post(mContext, obj, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int i, Header[] headers, byte[] bytes) {
                Log.e("获取极速聊信息--", new String(bytes));
                Gson gson = new Gson();
        ...
        client.post(url, requestParams, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int i, Header[] headers, byte[] bytes) {
                ...
                if (result.equals("0")) {
                    ...
                    ToastUtil.showMessageDefault(ShopDetailInfoActivity.this, leaveMeassageBean.getResultNote());
                }
            }
     ...
     */
    private void replayMessage(String commentContent, String commentId, String objectUserId, String objectId, String status) {
        JSONObject obj = new JSONObject();
        try {
    ...
    private void getGases(String proName) {
        ...
        Log.e("猜你喜欢提交数据--", obj.toString());
        HttpUtils.post(mContext, obj, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int i, Header[] headers, byte[] bytes) {
                Log.e("猜你喜欢返回值--", new String(bytes));
                Gson gson = new Gson();
     ...
     * @param count 数量
     */
    private void useBossTicket(final String count) {
        JSONObject obj = new JSONObject();
        try {
            obj.put("userName", mCurrentUserName);
            obj.put("count", count);
            obj.put("proId", mCommodityID);
            obj.put("cmd", "applyVouchers");
            ...
            public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
                ...
                try {
                    ...
                    if ("0".equals(obj.getString("result"))) {
                        ...
                        intent.putExtra("aboutus", chatlist.get(0));
                        startActivity(intent);
                        int total = Integer.valueOf(SharedPreferenceUtils.getBossTicketCount(ShopDetailInfoActivity.this)) - Integer.valueOf(count);
                        if (total >= 0) {
                            SharedPreferenceUtils.saveBossTicketCount(ShopDetailInfoActivity.this, String.valueOf(total));
                        }
                    } else {
                        Toast.makeText(ShopDetailInfoActivity.this, obj.getString("resultNote"), Toast.LENGTH_SHORT).show();
                    }
                } catch (JSONException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
    ...
    private void requestShopDetailInfo(String commodityID, String latitude, String longitude) {
        ...
        try {
            ...
            obj.put("userLong", latitude);
            obj.put("userLat", longitude);
        ...
        HttpUtils.post(mContext, obj, new AsyncHttpResponseHandler() {
            ...
            public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
                stopLoadingAnim();
                Log.d("mikes",
                        "ShopDetailInfoActivity requestShopDetailInfo back= "
                                + new String(arg2));
                Gson gson = new Gson();
                try {
                ...
                case GET_LEAVEMESSAGE:
                    if (list.size() > 0) {
                        if (list.size() < 4) {
                            ...
                            lmAdapter = new LeaveMeassageAdapter(ShopDetailInfoActivity.this, list,3);
                            messagemore.setVisibility(View.VISIBLE);
...
//                        lmAdapter.notifyDataSetChanged();
    ...
    private void initAllDetailView() {
        ...
        String proSurplus = String.format(this.getResources().getString(R.string.home_item_proSurplus), comAndReply.getMoreGroSurplus());
        ...
        String time = String.format(this.getResources().getString(R.string.goods_details_time_work), comAndReply.getWeekStart(), comAndReply.getWeekEnd(), comAndReply.getDayTimeStart(), comAndReply.getDayTimeEnd());
        ...
        String good = String.format(this.getResources().getString(R.string.goods_detail_good_score), String.valueOf(comAndReply.getGoodEvalNum()));
        commentgood.setText(good);
        String mindle = String.format(this.getResources().getString(R.string.goods_detail_medium_score), String.valueOf(comAndReply.getMediumEvalNum()));
        commentmindle.setText(mindle);
        String bad = String.format(this.getResources().getString(R.string.goods_detail_bad_score), String.valueOf(comAndReply.getBadEvalNum()));
